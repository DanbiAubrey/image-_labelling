...
class GenerateLabel(Label):
    def __init__(self, input_dir:Path, output_dir:Path, context_prompt_path:Path, llm_deployed_model = 'gpt-4o-t1'):
    ...
    def process_image(self, image_path:Path) -> dict:
        """Processes an image by encoding it into base64 and generate a label from LLM""" 
        base64_image = self.encode_image(image_path)
        if base64_image is None:
            raise ValueError("Failed to encode image: {image_path}")
    ...
    # Multi threading to mitigate latency
    # to fasten labelling process
    def label(self, max_worker:int=4):
        """Generates labels for images, using thereading for parallel execution"""

        files= list(self.input_dir.glob("*.jpg"))
        if not files:    
            print("No image found in: {self.input_dir}")
            return 
    ...
    def gather_label_results(self):
        ...
        label_files = self.output_dir_temp.glob('*.json')
        ...
        # cache: Save the results ti a JSON file
        self.output_dir.mkdir(exist_ok=True)
        tx = pytz.timezone('Asia/Seoul')
    ...
    def load_labels(self)->set:
        """Load hte labels from a JSON file"""
...
if __name__ == '__main__':
    context_prompt_path = Path("prompts") / 'label.md'
    input_dir = Path('frames')
    output_dir = Path('labels')

    generate_label = GenerateLabel(input_dir = input_dir,
                                    output_dir = output_dir, 
                                   context_prompt_path=context_prompt_path)
    
    generate_label.label(max_worker=5)
